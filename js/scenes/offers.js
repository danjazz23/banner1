...
const OffersScene = {
    ...
    init() {
        ...
        this.elements.timer3 = document.getElementById('timer-3');
    ...
    setupTimers() {
        ...
        // Timer 3: Happy Hour (4 hours 20 minutes from now)
        const now3 = new Date();
        now3.setHours(now3.getHours() + 4);
        now3.setMinutes(now3.getMinutes() + 20);
        this.config.endTime3 = now3.getTime();
    },
    ...
    updateTimers() {
        ...
        // Timer 3: Hours:Minutes:Seconds (Happy Hour)
        if (this.elements.timer3) {
            const distance3 = this.config.endTime3 - now;
            
            if (distance3 > 0) {
                const hours = Math.floor(distance3 / (1000 * 60 * 60));
                const minutes = Math.floor((distance3 % (1000 * 60 * 60)) / (1000 * 60));
                const seconds = Math.floor((distance3 % (1000 * 60)) / 1000);
                
                const hoursEl = this.elements.timer3.querySelector('.time-unit:first-child .num');
                const minutesEl = this.elements.timer3.querySelectorAll('.time-unit .num')[1];
                const secondsEl = this.elements.timer3.querySelector('.time-unit:last-child .num');
                ...
                // Reset timer when expired
                const newEnd = new Date().getTime() + (5 * 60 * 60 * 1000); // 5 hours from now
                this.config.endTime3 = newEnd;
            }
        }
    },
